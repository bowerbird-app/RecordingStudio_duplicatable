...
module RecordingStudioDuplicatable
  ...
  #   RecordingStudioDuplicatable.configuration.hooks.after_initialize do
  #     Rails.logger.info "RecordingStudioDuplicatable initialized!"
  #   end
  #
  ...
  class Hooks
    ...
    def log_hook_error(error, event_name, _hook)
      ...
      Rails.logger.error "[RecordingStudioDuplicatable::Hooks] Error in #{event_name} hook: #{error.message}"
    ...
    class << self
      ...
      # @return [Array] Results from all hooks
      def run(event_name, *)
        RecordingStudioDuplicatable.configuration.hooks.run(event_name, *)
      end
      ...
      def run_around(event_name, context, &)
        RecordingStudioDuplicatable.configuration.hooks.run_around(event_name, context, &)
      end
